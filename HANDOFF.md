...
> **2026-09-14 추가**: ③ 반자동 노출순위 **밤샘 라이브 완주 검증**(19:27~05:59, 22계정·**차단 0건**·271키워드, **48개 1~50위·최고 4위**, 나머지 50위 밖). vid 없는 상품 **상품명 매칭 폴백**(a01c01d)이 라이브에서 작동 — 노출명 17건 갱신·구글시트 통계 25시트/계정목록 반영. 함께 **버그 수정**: `rank._load_results` 가 Akamai 챌린지 감지 시 던지는 `RankBlocked` 메시지가 **미정의 `url`** 을 참조해 `NameError` 로 터지던 것 → `SEARCH_URL.format(...)` 로 실제 차단 URL 사용(자동 순위경로 서킷브레이커가 정상 동작하도록). 반자동은 이 함수를 안 타서 무관했음.
>
> **2026-09-14 추가2 — 계정목록 서식 + 반자동 대기 단축**: (1) 구글시트 `계정목록` **제목줄 전체 단색 통일**(진한 청회색 B7C9E8)·**사업자별 밴드색 8색**(중간 채도·**웜↔쿨 교대**로 대비, 흐릿함 개선)·**판매중지 행도 사업자 밴드색**(값 보존, 사업자별 동일색 유지)·라이브 재동기화로 27사업자 검증(`gsheet_index.py`, 커밋 6d6e72d·571dd4e·55f4e8f). (2) **반자동 실행시간 단축**: 밤샘 실측 결과 총시간의 **~86%가 검색 간 대기**(271키워드×평균120s) → `RANK_NAV_DELAY_MIN/MAX_SEC` **90~150→45~75**(평균120→60, 총 10.5h→~6h 예상). 대기를 **`_interruptible_sleep`(중단 반응) + '검색 앞'으로 이동**해 **마지막 검색 뒤 자투리 대기 제거**(`_track_ranks_semi`, `measured_any` 게이트). ⚠ 다음 밤도 무차단이면 30~50으로 한 단계 더(단계적 하향). **라이브 재검증 필요**(사무실/핫스팟).
...
**이번 세션 완료(전부 커밋)**:
1. **반자동 순위 무인화 전반**: stale URL(사용자 검색 미인식) 수정(`_live_url`=location.href 직접), 안내메시지 상황별 정확화, 키워드 복사오염 방지(공백분리)+검색창 **자동채움**(`_prefill_search`)+**자동검색 Enter**(`_submit_search`, `RANK_SEMI_AUTOSUBMIT=True`), **50위 초과 실제등수 기록**(`RANK_SCAN_MAX_SEMI=300`).
2. **순위 차단 대응 = 쿨다운-재개**(`52195d7`,`80e18db`): 연속 `RANK_SEMI_AUTO_MAX_MISS`(3)회 차단 시 **하드중단 대신 긴 쿨다운 후 자동재개**(`RANK_SEMI_COOLDOWN_SEC=1800`=30분), 진전 없으면 `RANK_SEMI_COOLDOWN_MAX`(4)회 후 당일중단. `_interruptible_sleep`(5분 하트비트).
3. **전체실행 무인화**(`1ea1839`): 무인 로그인(`LOGIN_UNATTENDED=True` — 차단·2차인증 시 창 안 띄우고 즉시 건너뜀, `LOGIN_UNATTENDED_WAIT_SEC=45`·`LOGIN_BLOCK_GRACE_SEC=8`), 날짜인식 재개(`resumable_progress`=오늘 started_at만 재개→날짜 바뀌면 처음부터), app_qt `do_run_full` 팝업 없이 자동결정.
4. **재실행=미완료분만**(`d7bf642`): 로그인 못한 계정 남으면 완료 시 진행분 **유지**(같은 날 재실행이 남은 것만). 전부 성공 시에만 정리.
5. **vid 없는 상품 명시 로그**(`c81b428` 등): 자동/반자동 둘 다 `[건너뜀] … vendorItemId 없음(①판매수집 미완)` + 개수 집계.
6. **UI 순위 간격/쿨다운 입력란**(`75f804d`): 설정탭에서 `RANK_NAV_DELAY_MIN/MAX_SEC`·`RANK_SEMI_COOLDOWN_SEC`·`RANK_SEMI_COOLDOWN_MAX`·`RANK_SEMI_AUTO_MAX_MISS`·`RANK_SEMI_AUTO_WAIT_SEC`를 코드수정 없이 조정(런타임 반영, 실행 중에도).
...
- ⚠️ **오늘 아침 첫 ① 실행이 "새통계"로 진행돼 기존 동결키워드(266개+과거이력)가 백업으로 밀림** → 백업 `output/쿠팡데이타분석_통계_보관_260911_131154.xlsx`. 현재 마스터는 오늘 새로 시작한 것(②가 새 키워드 생성함). **키워드 시계열 연속성 끊김** — 필요 시 백업 복원 검토(사용자는 A=새 키워드로 진행 선택함).
- ② 키워드: 완료(179키워드/46상품, 대부분 새로 생성).
- ③ 순위: **차단으로 중단**(오늘 IP 뜨거움, 22/179 측정=대부분 50위밖 실측값, 프로젝터마운트=2 등 일부 상위). 남은 157개. 진행분 저장→재실행 시 이어서. **쉰 IP(내일)/느린 간격 권장**.
- **vid 없는 상품 24/68**: 판매분석(vi-detail-search)이 그날 활동상품만 반환→미포함분 vid 없음. **재고 API(inventory, vid+productName 있음)로 대장 매칭 보강** 개선 미착수(제안됨).
- **판매중지 표기**(미구현, 설계만): 현재 [input_list.py:235] 판매중지 상품 추적제외 → 요구="결과파일에 판매중지로 표기"(A열 구분="판매중지"·데이터 공란 제안). 다음 ①부터 반영.

## 0-A. 최신 세션 (2026-09-09) — 이전 세션(참고)
