...
_ANALYZE_SYSTEM = (
    ...
    "core=가장 정확한 상품유형(한 단어). use=실제 사용목적(한 문장). "
    "identities=이 상품을 '바로 그것'이라 부를 수 있는 상품유형 동의어들(고객이 그 단어로 검색하면 이 상품을 "
    "기대하는 것들 — 예 화로테이블=바베큐테이블=숯불테이블=그릴테이블). 단 **구성부품·다른 형태는 제외**"
    "(화로테이블의 '바베큐그릴'·'화로대 스탠드'는 identities 아님). "
    "attributes=형태·재질·기능·장소 등 실제 속성. anchors=고객이 검색할 핵심 명사형 키워드(네이버 연관확장 씨앗). "
...
def _analyze_prompt(title: str, n: int) -> str:
    return (
        ...
        "- identities: 정체성(동의어 상품유형)들. 예 화로테이블·바베큐테이블·숯불테이블·그릴테이블.\n"
        f"- anchors: 최대 {n}개(정체성과 그 핵심 세부).\n"
        "- 없는 속성 추측 금지. 브랜드·수식어·수량·거대 일반어('캠핑용품') 제외.\n"
        '- 예: {"core":"화로테이블","use":"야외에서 둘러앉아 고기 굽는 원형 화로 테이블",'
...
_JUDGE_SYSTEM = (
    ...
    "DROP=다른 상품군·거대 일반 카테고리어('캠핑용품')·정보성·**타사 브랜드**(제목의 자사 브랜드가 아니면)·없는 속성. "
    "match(0~100)=이 상품과의 적합도. 검색량은 고려하지 말고 상품 적합성만 판단한다."
)
...
_SELECT_SYSTEM = (
    ...
    "현재 순위가 낮아도 검색수요가 크면 GROWTH로 적극 검토한다."
)
