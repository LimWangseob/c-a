...
def _analyze_prompt(title: str, n: int) -> str:
    return (
        ...
        "- identities: 정체성(동의어 상품유형)들. **핵심 성분·대상 자체와 그 주요 형태를 모두 포함**"
        "(예 '레몬버베나추출물정'이면 '레몬버베나'·'레몬버베나추출물'·'레몬버베나정'). 형태단독어·거대 일반어('건강환'·'건강식품') 제외.\n"
        f"- anchors: 최대 {n}개(정체성과 그 핵심 세부 — 핵심 성분 base 단어도 포함).\n"
