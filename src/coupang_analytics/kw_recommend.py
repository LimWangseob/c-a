...
def select_keywords_light(title: str, naver: NaverAdApi, ai_key: str | None,
    ...
    if exclude:   # 통계 유지 중 발굴 추가 — 이미 추적 중인 키워드는 후보에서 제외(새 것만 뽑음).
        ex = {e.strip() for e in exclude}   # 정확 표기로만 제외(띄어쓰기 유의미 — '캠핑 타프'≠'캠핑타프', 2026-09-17)
        relevant = [c for c in relevant if c.keyword.strip() not in ex]
        tiers = {k: v for k, v in tiers.items() if k.strip() not in ex}
