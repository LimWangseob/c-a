...
# 데이터 API(상품조회·재고·판매분석) **순수 응답 원문**을 매 실행 계정별 gzip 사이드카로 저장할지
# (소유자 2026-09-24). 가공 없이 그대로 남겨 재수집·재빌드 없이 다양한 오프라인 분석 가능
# (중복 판별·필드 탐색 등). 저장 위치=output/_raw/{계정}_{api}_p{n}.json.gz · run_log엔 위치만.
...
# 로켓그로스 입고 요약(관리대장 입력·선택) — 헤더 '최근입고 : …' 로 묶어 표시(소유자 2026-09-24). 정규화(소문자·공백제거) 별칭.
IN_ALIASES_INB_REQDATE = ("그로스요청일자/비고", "그로스요청일자", "요청일자", "그로스요청일")
IN_ALIASES_INB_REQQTY = ("요청수량",)
IN_ALIASES_INB_WORKQTY = ("작업수량",)
IN_ALIASES_INB_BOX = ("박스", "박스수")
IN_ALIASES_INB_PALLET = ("파레트", "파렛트", "팔레트", "pallet")
IN_ALIASES_INB_DONEDATE = ("완료일자", "완료일")
IN_ALIASES_INB_SHIPDATE = ("출고일자", "출고일")
