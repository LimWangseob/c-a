...
# 수동 교체용 프리셋(위험도↓ 순): 최안전=60~120(현재 기본) · 뜨거운/flag된 IP=45~95 · 새 IP(핫스팟)=25~50 · 급할 때=15~35.
# 순위추적은 KW_MAX_TRACK(=7)개/상품 안쪽이라, 넉넉히 잡아도 무인 야간 운용(서킷브레이커·쿨다운 동반)으로 소화됨.
RANK_NAV_DELAY_MIN_SEC = 60   # 최소 대기(초) — 최안전 기본(평균 ~90s, 매우 사람스러움)
RANK_NAV_DELAY_MAX_SEC = 120  # 최대 대기(초). 무작위
