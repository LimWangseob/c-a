...
M_RANK_OFF = "노출 순위(중단)"   # 담당자가 구글시트에서 지운 키워드 = 추적 중단(이력·과거값은 보존, 검색 제외)
...
# 한번 통계를 시작하면 AI 선정 키워드는 동결(시계열 의미 유지). 담당자가 구글시트에서 지운 키워드는
# **추적 중단**(M_RANK_OFF)하되 과거 이력·값은 보존한다(제거 아님). 상품당 활성 추적 상한 = KW_MAX_TRACK.
KW_MAX_TRACK = 10          # 상품당 **활성** 추적 키워드 상한(가변·config에서 조정). AI 발굴·담당자 수동 추가 공통 상한
