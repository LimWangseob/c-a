- [🔄코드건강 Tier A 완료 + Tier B B1·B2·B3 완료(2026-09-26)](handoff-code-health-tierA-260926.md) — 소유자 "전체 소스코드 점검(누더기·과거 정상→오류)"+"Tier B 전체 강행". **측정으로 회귀 확증**(9/22→9/26 pipeline+751·workbook+884줄, 4파일 D+ 8개 재유입·원인=게이트 구멍). **Tier A**(커밋 f90d913)=D+ 8개 행동불변 제자리분해+게이트 구멍 수정(4파일 D+ 0 유지 차단)+죽은코드. **Tier B**=pipeline **완전 해소**(원래 2684줄 C→**6모듈 전부 A/B**: pipeline.py A(20.94)·sales A·process A·ranks B·gsheet A·paths A). 커밋 7b08dc2·f28e34f·d7c54f8. 재배선 교훈=이동 함수의 내부 호출 심볼은 이동한 모듈서 patch·이중 소속(warmup·WingBrowser·select_keywords_light)은 양쪽·핀이 oracle. **workbook 도 분리 완료**(2460→workbook.py 1501[C·KNOWN_BAD, 응집 데이터모델] + workbook_common A·workbook_render A·workbook_index A). 복잡 렌더/인덱스 전부 A급 모듈로(mixin·monkeypatch 0·핀 oracle). 커밋 4b421a1. workbook.py 코어 B화는 diminishing returns 로 보류. **결과: 원래 두 C 몬스터(pipeline 2684·workbook 2460)가 전부 해소** — pipeline 6모듈 A/B·workbook 4모듈(코어만 허용C). 게이트 7종+복잡도 초록·행동 불변. 미푸시 커밋 다수. ⚠pipeline_sales 라이브(로그인) 1회 확인 권고.
