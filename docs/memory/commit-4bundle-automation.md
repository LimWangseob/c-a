...
metadata:
  ...
  modified: 2026-09-25T11:39:17.367Z
---
...
**push 때에도 적용(소유자 2026-09-25)**: **pre-push 훅**도 `sync_memory.py --stage` 실행 → 미커밋 메모리 변경이 있으면 **자동 커밋**(`--no-verify`, "메모리 스냅샷 자동 동기화(pre-push)")해 push 때에도 무손실. ⚠자동 커밋은 진행 중 push 의 ref 에는 안 들어가므로(git ref 는 push 시작 시 확정) "완료 후 다시 push" 안내 출력 — 단 정상 흐름은 pre-commit 이 이미 매 커밋 메모리를 커밋하므로 이 분기는 거의 안 탐. + `run_checks`(전체)+`check_complexity`.
