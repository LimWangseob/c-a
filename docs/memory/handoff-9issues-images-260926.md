...
metadata:
  ...
  modified: 2026-09-26T01:45:00.838Z
---
...
5. **항목2 상품명 하이퍼링크 productId 출처 확대**: ⚠상품조회(vendor-inventory)엔 공개 productId 없음(실측 _raw·vendorInventoryId만). 기존엔 ③순위 SERP 매칭 시만 확보(미매칭=검색링크 폴백). → **판매분석(vi-detail `vendorItemDetails.productId`) ∪ 재고(`listingDetails.productId`)** 로 수집 시 vid→productId. **실측: 재고 519 vid 중 306이 판매분석에 없음(판매 0 로켓그로스)=재고 pid 필수**. 배선: collector `OptionMetric.product_id`+`_parse_inventory_pids`·`fetch_inventory` **3→4튜플**·pipeline `pid_by_vid`(inv∪sales) → `_discover_products`→`_login_and_discover`(8튜플)→`_finish`→`_process_account`→`_ProcCtx`→`_process_option` `_apply_pid`. 판매자배송(NORMAL)도 판매활동 있으면 커버(zero-activity NORMAL만 검색 폴백·불가피).
