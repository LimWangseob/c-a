...
_LEDGER_NAME = "핀상품"
_VID = "vidPIN"
_LISTINGS: list = []   # fetch_vendor_inventory 반환(빈 리스트=업번들 없음) — 아래 두 함수에 전달·업번들 vid 순회 안전
...
def _install_collector_fakes(*, vendor_ok=True, discover_behavior="ok"):
    ...
    def fake_fetch_inventory(page, log):
        return {_VID: 42}, {_VID: _LEDGER_NAME}, {_VID: False}, {_VID: "900123"}   # 4번째=productId(항목2)
...
def pin_session_reuse():
    ...
    acct, metrics, inventory, sale_status, _ub, _lv, _vm, _pid = _run_login()
...
def pin_otp_skip():
    ...
    _install_collector_fakes()
    acct, metrics, inv, status, _ub, _lv, _vm, _pid = _run_login()
