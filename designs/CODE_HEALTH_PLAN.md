...
- ✅ **6a(커밋)**: `workbook_common.py`(leaf·A 51.5) — 상수·헬퍼·dataclass·openpyxl 재노출(`__all__`). workbook.py `import *`.
- ✅ **6b(커밋 4b421a1)**: `workbook_render.py`(`_RenderMixin`·**A 23.2**: 팔레트 상수+마이그·regroup·`_v4_*`·apply_style·`_style_*`) + `workbook_index.py`(`_IndexMixin`·**A 37.3**: `_build_index`/`_index_row`/`_idx_*`/promo_effect/`_sync_marketing`). `class OutputWorkbook(_RenderMixin, _IndexMixin)`. 팔레트 상수(_FILL_*·_FN)는 _RenderMixin 에(self 참조). monkeypatch 재배선 0. 게이트 7종+복잡도 초록·행동 불변.
- **결과**: workbook 2460 → workbook.py 1501(C·KNOWN_BAD) + common(A 253) + render(A 541) + index(A 252). **복잡한 렌더/인덱스 로직 전부 A급 모듈로**·새 모듈 D+ 0.
