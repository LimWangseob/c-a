...
- **🔄 정책 변경 — 실제 상호작용 재현 허용(2026-09-14 소유자 지시, "중요·지금부터 허용"):** 이전 "행동 위장(마우스/스크롤/체류시간) 금지"(2026-09-10)를 **철회**. 근거=타이핑(trusted 키이벤트)이 붙여넣기 대비 실측 통과(§4.8), 상호작용 진짜성이 변수일 수 있음(마우스/스크롤 효과는 미검증). **허용 범위 = 실제 브라우저의 실제 입력**(CDP `Input` 마우스 이동·휠 스크롤·실제 페이지 네비 = 타이핑과 동일 계열, 정상 입력 재현). **여전히 금지(별개·명시 지시 시만) = 기기 지문/TLS 위조**(§4.9 아래 §148: UA/sec-ch-ua 위장·JA3 조작·`_abck` 재생). ⚠ 도입 시 **깨끗한 IP(핫스팟) A/B로 실효성 먼저 검증**·위탁계정 정지 위험 유의(효과 가정 금지). 상세=메모리 [[rank-antiblock-circuit-breaker]].
