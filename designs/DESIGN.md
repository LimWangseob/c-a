...
## 0-0000000. 최신 반영 요약 (2026-09-20 후속2 — 담당자 키워드 편집 동기화 + 상한 10)

> 소유자 요구: 담당자가 결과 구글시트 키워드를 **직접 편집(추가·삭제·수정)** 한 것을 그대로 반영하되,
> **삭제한 키워드의 과거 이력은 보존**한다. 상품당 활성 상한 = **10개(가변·config)**.
> - **동기화(추가+중단+재활성)**: `gsheet_stats.merge_staff_keywords`가 실행 시작 시 구글시트 키워드 영역을
>   읽어 워크북과 맞춘다. ①시트에 있고 워크북에 없음 → **추가(활성)** ②워크북엔 활성인데 시트에서 사라짐 →
>   **추적 중단**(삭제 반영·행/과거값 보존·검색 제외) ③시트에 다시 나타난 중단 키워드 → **재활성**(이력 유지).
> - **중단 표현 = 순위행 G열 라벨 `M_RANK_OFF`("노출 순위(중단)")** — 행·과거 순위값은 그대로 두고 라벨만 바꾼다.
>   `_reindex`가 M_RANK/M_RANK_OFF 둘 다 키워드행으로 등록(프리즈 카운트·이력 유지), `is_keyword_inactive`/
>   `active_keywords`/`set_keyword_active`(workbook). 순위 검색·검색량 채움은 **active_keywords만**(run_full 동결분기·
>   select_keywords_stage·track_ranks_stage·_track_ranks_semi 전부). `read_staff_keywords`는 중단행(G=M_RANK_OFF)을
>   건너뛰어 **재활성 무한루프 방지**. apply_style이 중단 키워드를 회색으로 표시. 1:1 미러 유지(마커가 값이라 미러링됨).
> - **상한 = KW_MAX_TRACK 7→10**(가변). AI 발굴·담당자 수동 추가 공통 활성 상한. 삭제된(중단) 키워드는 상한 카운트 제외.
> - **삭제 슬롯 AI 재충전 방지**: 프리즈 카운트(`product_keywords`)는 중단 키워드도 포함 → 담당자가 지운 자리를 AI가 다시 안 채운다.
> - **안전장치**: 그 상품 시트에서 활성 키워드가 하나도 안 읽히면(파싱 실패·빈 영역) 그 상품은 건드리지 않음(오판 전체중단 방지).
>   정상 미러 상태(시트=마스터 활성)면 동기화는 no-op. ⭐라이브 실측: 실제 구글시트에 담당자 편집 2건 대기(휴라엘 거북목교정기·캠핑타프) 확인.
> 검증: verify_offline·simulate·verify_gsheet + kw-sync 표적(추가/삭제=중단+이력보존/재로드왕복/재활성/상한10/안전장치) + 실제 마스터·시트 비파괴 확인.
...
- **라벨 형식 = 년도 없는 '월.일'(예 09.16), 당분간**(사용자 요청 2026-09-16). `col_label`·`ensure_date` 갭필·`normalize_date_columns` 모두 `%m.%d`, `_parse_date`가 `%m.%d`(올해)도 해석. 기존 `yy.mm.dd`도 normalize가 날짜로 매칭해 재포맷(값 유실0). ⚠연말/연초 경계 당분간 미고려.
