...
**블록 헤더 레이아웃 재구성 v4(소유자 확정 2026-09-24 · ⏳미구현 — 새 세션서 구현):** 좌측 `A:B`를 **라벨 칸**(지금 `G`가 지표 라벨인 것과 대칭)으로 바꿔 우측 지표 7줄과 정렬한다. `A:B` 라벨 = 상품명(2줄 세로병합)·VID·판매방식·로켓그로스(3줄 세로병합), `C:F` = 값. 로켓그로스 3줄 = ①`쿠팡 등록 로켓그로스 판매일` ②`그로스요청일자 + 출고일` ③`요청수량·작업수량·박스·파레트`. 판매자배송(개인)은 로켓그로스 3줄 생략. **VID는 상품명과 분리된 줄**(소유자 A안) → vid 저장을 헤더 이름칸 → 숨김 메타시트(`_상품ID` col3 재활성)로 이전(2026-09-20 "vid=헤더셀" 되돌림·근거=소유자 A안·핀 먼저·옛 마스터는 `_reindex`가 헤더 'VID :' 꼬리서 폴백 마이그레이션). 배경색=**좌측 라벨 칸에 상품군 색(살구↔민트 교대)**=라벨+상품군 구분 겸용, 지표 라벨(G)=연파랑 유지. 좌측 라벨 칸 폭=지표 라벨 칸 폭 통일. 키워드 소헤더의 사업자명(A) 제거→키워드 A~E 좌측 확장·행 높이 통일. 목차 복귀 링크 문구=`👈 계정목록으로 이동`(✅반영). 상세 구현 계획=메모 `handoff-block-layout-redesign`(workbook.py 11곳). ✅이미 반영=링크 문구만.
