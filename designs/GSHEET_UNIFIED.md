...
이걸 **구글 시트로** 합친다(⚠️ 아래 옛 '1개 시트' 초안은 위 §★ 확정 모델의 **파일 2개**[입력 관리대장 + 출력 결과시트]로 정정됨 — 아래 문단은 역사적 기록):
...
- `ui/app_qt.py` — 설정 탭의 rclone "결과 파일을 구글드라이브로 공유하기"는 **제거 완료**(결과가 이미 구글 시트, `_upload_to_gdrive` 삭제). 설정 탭은 "구글 시트 연동" 카드로 대체됨.
...
- [x] **삭제/판매중지 감지 = 취소선 또는 관리대장 '상태' 컬럼**(둘 중 하나면 제외). `IN_ALIASES_STATUS`/`IN_STATUS_DISCONTINUED`.
      구글시트도 취소선을 **읽는다**(`GSheetClient.read_strike_grid` → Sheets API `effectiveFormat.textFormat.strikethrough`
...
**확정 스키마 = openpyxl `계정 목록`과 동일 8열**(대표자 컬럼 추가 2026-09-17):
`A 대표자 | B 사업자 | C 상품명(클릭 이동·노출명·통계시트 하이퍼링크) | D 계정ID | E 체험단시작 |
F 체험단종료 | G 모니터링종료 | H 상태`. **자동열=A·B·C·D·H**(프로그램), **직원 입력열=E·F·G**(온라인 편집, 프로그램 미접촉).
- 대표자(A) = 관리대장 대표자명(`wb.representative_of`). 안정 키 메모는 **B(사업자)** 셀에 저장.
- **기존 7열 시트 자동 마이그레이션**: `sync_index`가 헤더에 대표자가 없으면 `_ensure_rep_column`으로 **A에 빈 열 1개 삽입**(insertDimension COLUMNS 0) → 모든 값·메모·서식·직원 마케팅이 우측으로 밀려 새 위치와 정확히 일치. `read_marketing`은 헤더 라벨로 열을 탐지해 옛/새 레이아웃 모두 안전.
...
- [x] **쓰기 규칙(사용자 확정)**: 자동열만 갱신 · **신규 상품은 계정별 그룹 맨 마지막**에 `insertDimension`으로
      빈 행 삽입(기존 마케팅 행이 통째로 밀려 D~F 보존) · 새 계정은 맨 아래 · **판매중지(대장에 줄 남음)=상태 ⛔ + 사업자 밴드색**(값 보존, 2026-09-14).
- [x] **계정 삭제(2026-09-17)**: 관리대장에서 **줄이 완전히 사라진 계정**은 결과에서 **완전 삭제** — `gsheet_index.delete_accounts(client, [(사업자,계정ID)…])`가 계정목록 행(계정ID 열 헤더 탐지·아래→위 deleteDimension) + 그 사업자 통계 시트(deleteSheet) 제거. 워크북은 `delete_account`(시트·이력·메타). 판정=계정ID가 `InputList.ledger_account_ids`(줄 존재·판매중지 포함)에 없음 → run_full이 `_push_gsheet(removed_accounts=)`로 전달. 판매중지로 남은 건 삭제 아님(유지+경고). ⚠되돌릴 수 없음(이력 소멸).
- [x] **행 매칭 = 안정 키**(계정ID+vid 앵커, 노출명 바뀌어도 불변)를 A열 셀 **메모(note)** 에 저장/조회.
- [x] **마케팅 D~F 값은 어떤 경로에서도 안 씀**(단위테스트로 불변식 검증). 동시편집 안전.
- [x] 서식: 제목 병합·**제목줄 전체 동일 헤더색**(진한 청회색 B7C9E8·굵게)·**계정ID별 밴드색 8색 순환**(2026-09-17, 행 전체 동일·인접 계정 다름·같은 대표자라도 계정ID 다르면 색 다름·중간 채도 **웜↔쿨 교대**=대비)·틀고정(2행)·열너비·상품명 HYPERLINK. **판매중지 행도 그 계정 밴드색 + 대표자 채움**(공란 방지, 2026-09-17). (2026-09-14: 헤더 열별 이색·마케팅 안내색 FFF2CC 폐지 → 제목줄 단색 통일; 밴드 2색→8색, 흐릿→중간채도 웜/쿨 교대로 대비 강화.)
...
## 5. 데이터 모델 / 시트 스키마 (✅ 확정 — 출력 `계정목록` 8열, 대표자 추가 2026-09-17)

**출력** 결과 구글시트의 `계정목록` 시트(=openpyxl `계정 목록`과 동일 8열). **입력(관리대장)과 별개 파일.**

| 열 | 성격 | 내용 |
|---|---|---|
| A 대표자 | **자동** | 관리대장 대표자명(2026-09-17 추가) |
| B 사업자 | **자동** | 계정 로스터(관리대장 미러). 안정 키 메모를 이 셀에 보존 |
| C 상품명(클릭 이동) | **자동** | 매칭된 실제 노출명 + 통계시트 HYPERLINK(노출명·블록링크가 이 한 열에 접힘) |
| D 계정ID | **자동** | |
| E 체험단 시작일 | **직원 입력** | 온라인 편집 — 프로그램 미접촉 |
| F 체험단 종료일 | **직원 입력** | |
| G 모니터링 종료일 | **직원 입력** | |
| H 상태 | **자동** | 예정/체험단중/모니터링/종료/미수집/⛔ 판매중지 |
...
- Phase 2 — `input_list.py`: `_parse_grid`(공용)·`parse_input_rows(rows, strike_grid=)`·`parse_password_rows`·`read_ledger_rows`.
  **삭제/판매중지=취소선(`read_strike_grid`) 또는 관리대장 '상태' 컬럼**(`config.IN_ALIASES_STATUS`/`IN_STATUS_DISCONTINUED`).
