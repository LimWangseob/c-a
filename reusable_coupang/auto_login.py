"""자동 로그인 모듈 — 실제 Chrome + CDP 제어로 쿠팡(Akamai 봇탐지)을 통과해 로그인한다.

[reusable_coupang] 다른 프로젝트에서 `from reusable_coupang.auto_login import WingBrowser` 로 가져다 쓴다.

━━━ 왜 이렇게 만드는가(핵심 원리) ━━━
Playwright/셀레니움이 브라우저를 '띄우면' navigator.webdriver 등 자동화 흔적이 남아 Akamai 가 로그인을
막는다(Access Denied). 그래서 이 모듈은 **자동화 플래그 없는 실제 Google Chrome** 을
`--remote-debugging-port` 로 직접 구동하고 **CDP(Chrome DevTools Protocol)** 로 붙는다:
  1) 최초 로그인: 창을 보이게 띄우고 ID/비번을 **사람처럼 한 글자씩 실제 키보드로** 자동입력(붙여넣기 금지)
     → 제출. 2차인증/봇챌린지가 뜨면 그 창에서 사람이 처리(수동 폴백).
  2) 이후 재사용: **같은 프로필 폴더**(`--user-data-dir`)로 다시 열면 로그인 세션이 그대로 이어진다.
     (쿠키만 주입한 새 브라우저는 데이터 API 가 403 → 반드시 '사람이 로그인한 그 프로필' 재사용.)

━━━ 의존 ━━━
- playwright(sync) + 실제 설치된 Google Chrome.  `pip install playwright` (브라우저 바이너리 설치 불필요 — 시스템 Chrome 사용).
- `human_typing`(같은 패키지) — 로그인 입력을 신뢰 키이벤트로 타이핑.
- ⚠ Windows 전용 코드가 일부 있음(화면 중앙좌표=ctypes.windll, 잔여 Chrome 정리=PowerShell).
  다른 OS 로 옮기려면 `_center_pos`/`_kill_profile_chrome`/`reap_orphan_chrome` 를 그 OS 방식으로 교체.

━━━ 최소 사용 예 ━━━
    from reusable_coupang.auto_login import WingBrowser, WING_URL
    with WingBrowser(profile_dir="data/acct_foo", offscreen=False) as b:
        ...
        if not b.authenticated():
            ...
            b.wait_for_login(on_log=print, on_need_user=b.show)  # 2차인증 필요 시 창 표시
        assert b.authenticated()   # 로그인 완료(윙 대시보드 + 인증쿠키)
"""
...
def find_chrome() -> str:
    """설치된 Google Chrome 실행파일 경로. (다른 OS 는 후보 경로를 그 OS 에 맞게 바꿀 것.)"""
...
def _kill_profile_chrome(profile_dir: str) -> int:
    """이 user-data-dir 를 쓰는 잔여 Chrome 만 종료(이전 실행에서 안 닫힌 것). 종료 수 반환. [Windows 전용]
    ...
    try:
        ...
        print(f"[auto_login] 잔여 Chrome 정리 건너뜀({exc.__class__.__name__})")
        return 0
    return r.stdout.count("K")


def reap_orphan_chrome(data_dir: str = "data") -> int:
    """우리 자동화가 띄운 **잔여(좀비) Chrome 을 전부** 종료. 종료 수 반환. 앱 시작 시 1회 호출. [Windows 전용]

    `__exit__`의 `_kill_tree`는 정상 종료 때만 돈다. 앱이 taskkill/F·크래시로 죽으면 그 순간 열려 있던
    Chrome 이 좀비로 남는다(과거 246개 누적). 이걸 다음 시작 때 싹 정리해 **누적을 원천 차단**한다.
    우리 프로필 루트(`data\\...`)를 `--user-data-dir` 로 쓰고 `--remote-debugging-port` 로 뜬 Chrome 만
    종료 → 사용자의 일반 브라우징(기본 프로필)은 안 건드린다.
    """
    ...
    try:
        ...
        print(f"[auto_login] 좀비 Chrome 정리 건너뜀({exc.__class__.__name__})")
        return 0
    return r.stdout.count("K")


def _wait_port(port: int, timeout: float = 20.0) -> None:
    """Chrome 디버깅 포트가 열릴 때까지 대기(안 열리면 TimeoutError)."""
...
def _center_pos(w: int, h: int) -> tuple[int, int]:
    """화면 정중앙 좌상단 좌표(Windows). 로그인 창이 화면 밖으로 숨지 않도록. [Windows 전용]"""
...
def is_logged_in(url: str) -> bool:
    """윙 대시보드 도달 = 로그인. xauth(로그인 폼)·sso(전환 리다이렉트) 는 아직 미완."""
...
class WingBrowser:
    ...
    with 블록 진입 시 Chrome 을 띄우고 CDP 로 연결, 나갈 때 프로세스 트리를 정리한다.
    - profile_dir: `--user-data-dir`. **계정마다 다른 폴더**를 쓰면 세션이 계정별로 유지된다.
    - offscreen=True: 창을 화면 밖(-2400,-2400)에 둠(무인 동작). 로그인/2차인증엔 `show()` 로 표시.
    - port=None: 실행마다 빈 포트 자동 할당(고정 포트 재사용 시 ECONNRESET).
    """
    ...
    def __enter__(self) -> "WingBrowser":
        ...
        if killed:
            print(f"[auto_login] 이 프로필의 잔여 Chrome {killed}개 정리(프로필 잠금 해제)")
            time.sleep(1.0)
        ...
        # ⚠ 상시가동 안정성: __enter__ 도중 예외가 나면 파이썬은 __exit__ 를 부르지 않는다 → 이미 띄운
        # Chrome(_proc)·playwright 가 좀비로 남는다. 그래서 실패 시 여기서 직접 정리 후 재전파.
        try:
            try:
    ...
    def __exit__(self, *exc) -> None:
        try:
            ...
            print(f"[auto_login 정리] pw {e.__class__.__name__}")
        self._kill_tree()
    ...
    def show(self) -> None:
        """숨긴 창을 화면 정중앙으로 이동·표시하고 **맨 앞으로** 가져온다(로그인/2차인증 등 사람 조작용)."""
    ...
    def to_front(self) -> None:
        ...
        try:
            ...
            print(f"[auto_login] to_front 스킵({exc.__class__.__name__})")
    ...
    def _kill_tree(self) -> None:
        """Chrome 프로세스 트리 전체 종료(자식 렌더러까지). 안 그러면 Chrome 이 쌓여 메모리 먹통. [Windows: taskkill]"""
        if not self._proc:
            return
        try:
            ...
            try:
                ...
                print(f"[auto_login 정리] kill {e.__class__.__name__}")

    # ── 동작 ────────────────────────────────────────────────
    def goto(self, url: str, timeout: float = 30000) -> str:
        """URL 로 이동(domcontentloaded 까지만 대기 = 이미지 안 기다림). 최종 URL 반환."""
    ...
    def authenticated(self) -> bool:
        ...
        ⚠ URL 단독·쿠키 단독 판정은 과거 오판 버그의 원인 → 반드시 둘 다(AND).
        """
    ...
    def classify_login(self) -> tuple[str, str]:
        ...
        - error   : #input-error 에 메시지 표시(비번오류·계정잠금·휴면·OTP5회잠금) = 확정 실패
    ...
    def _check_remember_me(self, fr, log) -> None:
        ...
        재로그인 없이 세션 복원 가능(안 켜면 세션쿠키라 종료 시 소멸 = 매번 재로그인→차단 유발).
    ...
    def autofill_login(self, account_id: str, password: str, on_log=None) -> bool:
        ...
        ⚠ 비밀번호는 호출부가 **안전 저장소(예: OS 자격증명 관리자·DPAPI)에서** 그때만 받아 넘길 것.
           평문 파일·소스·로그에 남기지 말 것.
        """
    ...
    def wait_for_login(self, timeout: float = 300.0, poll: float = 2.0, on_log=None,
        ...
        """사람이(또는 자동입력이) 로그인할 때까지 대기. 완료 시 True.

        원인을 즉시 확정하도록 화면을 분류(classify_login)해서:
        - error(비번오류·계정잠금 등): 확정 실패 → 5분 안 기다리고 즉시 중단 + 증거 스냅샷(계정잠금 방지).
        ...
        (숨긴 창을 그때만 표시하는 용도 — 예: on_need_user=self.show). 자동입력만으로 되면 호출되지 않는다.
        """
        log = on_log or (lambda m: None)
        end = time.time() + timeout
        last_log = 0.0
        ...
        while time.time() < end:
            try:
                ...
                if code == "form":
                    ...
                    form_since = None
                    if code == "blocked":
                        # Akamai 접근차단은 IP 플래그라 사람 재로그인도 대부분 못 뚫는다(실측).
                        # 창 1회 표시로 기회를 주되, 300초 무한대기(→사람이 창 수동종료→드라이버 크래시)
                        # 대신 grace 후 스스로 건너뛴다.
