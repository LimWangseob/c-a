...
- **재부팅 자동복구(2026-09-16)**: Windows 업데이트 등으로 야간 재부팅 시 중단분을 이어서. ①전체실행/무인이 단계마다 `write_run_stage`로 `_실행단계.json` 기록 → 작업 스케줄러 **'로그온 시' 트리거가 `--resume`** 실행(`deploy/install_schedule.ps1`) → `start_resume`이 마커+진행중 파일 보고 **판매수집 스킵·순위부터** 이어서(없으면 즉시 종료). 무인 동작하려면 **Windows "재시작 후 자동 로그인 완료"** 설정 필요(세션 복원). 예방=업데이트 활성시간(17~9시). 계기=09-16 01:29 업데이트 재부팅으로 전체실행(③순위 59%)이 중단됨.
